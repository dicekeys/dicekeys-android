        ...
        binding.buttonSave.setOnClickListener {
            when(binding.keystoreType.checkedRadioButtonId){
                R.id.unlock_screen_lock -> {
                    biometricsHelper.encrypt(viewModel.diceKey.value!!,  AppKeyStore.KeyStoreCredentialsAllowed.ALLOW_BIOMETRIC_OR_KNOWLEDGE_BASED_AUTHENTICATION, this)
                }
                R.id.unlock_device_credentials -> {
                    biometricsHelper.encrypt(viewModel.diceKey.value!!,  AppKeyStore.KeyStoreCredentialsAllowed.ALLOW_ONLY_KNOWLEDGE_BASED_AUTHENTICATION, this)
                }
                R.id.unlock_biometrics -> {
                    biometricsHelper.encrypt(viewModel.diceKey.value!!,  AppKeyStore.KeyStoreCredentialsAllowed.ALLOW_ONLY_BIOMETRIC_AUTHENTICATION, this)
                }
            }
        }
