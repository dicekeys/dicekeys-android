...
JNIEXPORT jstring JNICALL
Java_org_dicekeys_crypto_seeded_SigningKey_keyDerivationOptionsJsonGetterJNI(
  JNIEnv *env,
    jobject thiz
  ) {
  try {
    return stringToJString(env,
      getNativeObjectPtr<SigningKey>(env, thiz)->keyDerivationOptionsJson
    );
  } catch (...) {
    throwCppExceptionAsJavaException(env, std::current_exception());
    return NULL;
...
JNIEXPORT jlong JNICALL
Java_org_dicekeys_crypto_seeded_SigningKey_constructJNI__Ljava_lang_String_2Ljava_lang_String_2(
  JNIEnv *env,
  jclass clazz,
  jstring seed_string,
